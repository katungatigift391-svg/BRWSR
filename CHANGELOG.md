...
* **Continuous Multi-Zap Mode (`Ctrl+Shift+Z`)**: Point-and-click overlay vaporizing with automated CSS selector derivation and persistent rule storage in `rules.json`.
...
* **Live In-Page Media Inspector (`Ctrl+Shift+M`)**: Interactive grid overlay with live preview cards, resolution metadata, and 1-click batch downloading.
* **Zero-Modal Auto-Save Engine (`Ctrl+J`)**: Background downloading with real-time transfer speed tracking and customizable destination directories.
